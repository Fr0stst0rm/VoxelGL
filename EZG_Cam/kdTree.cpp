...
KDTree::Node* KDTree::buildTree(typename std::vector<Vertex>::iterator start, typename std::vector<Vertex>::iterator end, int currLevel) {
	if (start >= end) return NULL; // empty tree

	//std::cout << "Lambda: \n";
	int axis = currLevel % m_dimension; // the axis to split on
	auto cmp = [axis](const Vertex& p1, const Vertex p2) {
	...
	std::vector<Vertex>::iterator mid = start + len / 2;
...
void KDTree::drawWireframe(const Shader* shader)
{
	...
	//Start rec
	drawWireframeRecursive(shader, m_minBound, m_maxBound, m_root, 0);
}

void KDTree::drawWireframeRecursive(const Shader* shader, glm::vec3 minBound, glm::vec3 maxBound, Node* node, int currLevel) {
	...
	if (node->m_left != nullptr) {
		...
		drawWireframeRecursive(shader, minBound, newMax, node->m_left, currLevel +1);
	}
	if (node->m_right != nullptr) {
		glm::vec3 newMin = minBound;
		minBound[node->m_axis] = node->m_cutingEdge; //TODO Check ob das mit [] überhaupt richtig ist
		drawWireframeRecursive(shader, newMin, maxBound, node->m_right, currLevel + 1);
	}
}
...
KDTree::Node* KDTree::findNode(typename KDTree::Node* currNode, const Vertex& pt) const {
	if (currNode == NULL || currNode->m_point == pt) return currNode;

	const Vertex& currPoint = currNode->m_point;
...
bool KDTree::contains(const Vertex& pt) const {
	auto node = findNode(m_root, pt);
	return node != NULL && node->m_point == pt;
}


void KDTree::insert(const Vertex& pt) {
...
const Vertex& KDTree::at(const Vertex& pt) const {
	auto node = findNode(m_root, pt);
	if (node == NULL || node->m_point != pt) {
		throw std::out_of_range("Vertex not found in the KD-Tree");
	}
	else {
		return node->m_point;
	}
...
Vertex& KDTree::at(const Vertex& pt) {
	const KDTree& constThis = *this;
	return const_cast<Vertex&>(constThis.at(pt));
}


Vertex& KDTree::operator[](const Vertex& pt) {
	auto node = findNode(m_root, pt);
	if (node != NULL && node->m_point == pt) { // pt is already in the tree
		return node->m_point;
	}
	else { // insert pt with default Vertex value, and return reference to the new Vertex
		insert(pt);
...
/*
void KDTree::nearestNeighborRecurse(const typename KDTree::Node* currNode, const Vertex& key, BoundedPQueue<Vertex>& pQueue) const {
	if (currNode == NULL) return;
	const Vertex& currPoint = currNode->m_point;
	...
	// Recursively search the half of the tree that contains Vertex 'key'
	int currLevel = currNode->m_level;
	bool isLeftTree;
...
/*
Vertex KDTree::kNNValue(const Vertex& key, std::size_t k) const {
	BoundedPQueue<Vertex> pQueue(k); // BPQ with maximum size k
	if (empty()) return Vertex(); // default return value if KD-tree is empty

	// Recursively search the KD-tree with pruning
	nearestNeighborRecurse(m_root, key, pQueue);

	// Count occurrences of all Vertex in the kNN set
	std::unordered_map<Vertex, int> counter;
	while (!pQueue.empty()) {
